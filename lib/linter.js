...
function warningNudge(diagnostics) {
  ...
  for (const diagnostic of diagnostics) {
    ...
    const separator = diagnostic.message.indexOf(" — ");
    const guidance = separator === -1 ? diagnostic.message : diagnostic.message.slice(separator + 3);
    const current = grouped.get(diagnostic.ruleId) ?? {
      ruleId: diagnostic.ruleId,
      message: guidance,
      count: 0,
    };
    current.count += 1;
    grouped.set(diagnostic.ruleId, current);
  }
  if (grouped.size === 0) return "";
  const lines = [...grouped.values()].map((item) => {
    const noun = item.count === 1 ? "warning" : "warnings";
    return `- ${item.ruleId} (${item.count} ${noun}): ${item.message}`;
  });
  return [
    "Slop Detector session guidance from your previous reply:",
    ...lines,
    "Avoid repeating these patterns in this reply. This is advisory; answer the user's current request normally.",
  ].join("\n");
}
...
function revisionFeedback(diagnostics) {
  ...
  const lines = diagnostics.map((item) => {
    const separator = item.message.indexOf(" — ");
    const guidance = separator === -1 ? item.message : item.message.slice(separator + 3);
    return `- ${item.level.toUpperCase()} ${item.ruleId}: ${JSON.stringify(item.text)} — ${guidance}`;
  });
  return [
    "Slop Detector rejected this output. Revise the flagged wording without changing its meaning or omitting required information:",
    ...lines,
    "Return only the revised output. Do not mention the lint pass.",
  ].join("\n");
}
